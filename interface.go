...
func (e *Elephant) Update(inputType reflect.Type, key int64, input interface{}) error {
	originalKey, err := getKey(inputType, input)
	if err != nil {
		return err
	}
	if originalKey != key {
		return fmt.Errorf("Key changes are not supported")
	}
